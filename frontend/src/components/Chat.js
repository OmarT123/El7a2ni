...
function Chat({ partner, name, setChat }) {
  ...
  useEffect(() => {
    const joinRoom = async () => {
      if (partner !== "") {
        const response = await axios.get("/getChattingRoom", {
      ...
      {collapse && (
        <div className="chat-header">
          <p onClick={handleCollapse}>
            {" "}
            {name} {!collapse && <ChatBubbleIcon sx={{ height: "15px" }} />}
            {collapse && <ChatIcon sx={{ height: "15px" }} />}
            {
              <CloseIcon
                sx={{ height: "15px", marginLeft: "150px" }}
      ...
      {!collapse && (
        ...
        >
          <p onClick={handleCollapse}>
            ...
            {
              <CloseIcon
                sx={{ height: "15px", marginLeft: "168px" }}
                onClick={() => setChat(false)}
              />
            }
          </p>
            ...
            {messageList.map((message) => {
              return (
                <div
                  className="message"
                  id={
                    partner === message.author || message.status === "Sent"
                      ? "other"
                      : "you"
                  }
                >
