...
const CheckoutHealthPackage = ({ user }) => {
  ...
  const payWithWallet = async () => {
    ...
    if (selectedFamilyMember === '') {
      ...
      localStorage.setItem('subscriberName', selectedFamilyMember);
      await axios.get("/payWithWallet", { params: body }).then(res => {if (res.data.success)window.location.href = res.data.url;else alert(res.data.message)}).catch(err => console.log(err));
    }
  };
