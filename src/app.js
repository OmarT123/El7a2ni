...
schedule.scheduleJob('0 0 * * *', async () => {
  ...
  try {
    ...
    for (const appointment of upcomingAppointments) {
      ...
      addNotification('Doctor', appointment.doctor, 'Appointment Reminder', `You have an upcoming appointment at ${appointment.date} with patient ${appointment.attendantName}.`, expiryTime, notificationDate)
      addNotification('Patient', appointment.patient, 'Appointment Reminder', `You have an upcoming appointment with Dr.${appointment.doctor.name} on ${appointment.date}.`, expiryTime, notificationDate)
...
app.get("/patientRetrieveNotifications", getUserFromTokenMiddleware, patientRetrieveNotifications)
...
app.get("doctorRetrieveNotifications", getUserFromTokenMiddleware, doctorRetrieveNotifications);
