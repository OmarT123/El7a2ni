...
const searchMedicine = async (req, res) => {
  ...
  if (req.query.medicinal)
    query["medicinalUse"] = new RegExp(req.query.medicinal, "i");
  try {
    const results = await medicineModel.find(query);
    if (results.length == 0) {
      res.json({ success: false, title: "Medicine not Found!", message: 'Try searching with another name/medicinal Use' });
    } else {
